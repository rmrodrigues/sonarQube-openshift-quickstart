...
var R=(this.wHeight==null||this.wHeight<=0?80:this.wHeight)-40;
var P=R-O;
...
function showMessage(B,A){$j("#"+B+"msg").html(A);
$j("#"+B).show()
...
};function updateDuplicationLines(C,D,F,E,B,A){$j("#duplGroup_"+D+" p.selected").removeClass("selected");
$j("#duplCount-"+D+"-"+F).addClass("selected");
$j("#duplFrom-"+D+"-"+F).addClass("selected");
$j("#duplName-"+D+"-"+F).addClass("selected");
$j("#duplLoading-"+D).addClass("loading");
if($j("#source-"+D+" :first-child").hasClass("expanded")){A=B+E-1
}$j.ajax({url:C+"&to_line="+A+"&from_line="+B+"&lines_count="+E+"&group_index="+D,success:function(G){$j("#source-"+D).html(G)
},type:"get"});
return false
};function loadResourceViewer(F,D,A,E,C){if(A==undefined){A=true
}var B=baseUrl+"/resource/index/"+F+"?tab="+D+"&display_title="+A+"&period="+E;
...
D.find(".issue-technicaldebt").slideUp("fast");
...
}function toggleTechnicalDebt(B){var D=$j(B).closest("[data-issue-key]");
var C=D.find(".issue-technicaldebt");
if(C.is(":visible")){C.slideUp("fast")
}else{D.find(".issue-changelog").slideUp("fast");
D.find(".issue-rule").slideUp("fast");
var A=D.attr("data-issue-key");
$j.get(baseUrl+"/issue/technicaldebt/"+A,function(E){C.html(E);
C.slideDown("fast")
})
}return false
